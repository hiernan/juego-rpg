extends ItemList
signal drop_on_healer(id: String)  # comprar: shop → inv
...
func _get_drag_data(at_position: Vector2):
	...
	return {"id": id, "from": "healer_inv"}

func _can_drop_data(at_position: Vector2, data) -> bool:
	return typeof(data) == TYPE_DICTIONARY \
		and data.has("id") and data.has("from") \
		and String(data["from"]) == "healer_shop"

func _drop_data(at_position: Vector2, data) -> void:
	if not _can_drop_data(at_position, data): return
	emit_signal("drop_on_healer", String(data["id"]))  # comprar 1
