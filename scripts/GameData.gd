...
var avatar := {
	...
	"inventory": [ "short_sword", "leather_armor", "leather_armor", "potion_small", "potion_small", "potion_medium", "potion_great" ],
...
#func get_item_display_name(id: String) -> String:
	## Devuelve un nombre amigable según el kind; sin usar `items`.
	#var kind: String = get_item_kind(id)
	#if kind == "weapon":
		#var w: Dictionary = weapons.get(id, {})
		#return String(w.get("name", id))
		...
		#var a: Dictionary = armors.get(id, {})
		#return String(a.get("name", id))
	## Otros kinds: si no hay catálogo, devolvemos el id como fallback.
	#return id
...
func _load_items(path: String) -> void:
	...
	# Normalizar nombres esperados (id,name,kind,subkind,heal_pct,heal_hp,stackable,max_stack,rarity,tooltip_extra)
	var idx := {}
	for i in range(headers.size()):
