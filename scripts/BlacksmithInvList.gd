extends ItemList
signal drop_on_blacksmith(id: String)
...
func _get_drag_data(at_position: Vector2) -> Variant:
	...
	return { "id": id, "from": "inventory" }

func _can_drop_data(at_position: Vector2, data: Variant) -> bool:
	if typeof(data) != TYPE_DICTIONARY: return false
	var id := String(data.get("id", ""))
	var from := String(data.get("from", ""))
	if id == "" or from != "shop_blacksmith": return false
	# armas/armaduras no stackean; cualquier arma/armadura es válida
...
func _drop_data(at_position: Vector2, data: Variant) -> void:
	var id := String(data.get("id", ""))
	if id == "": return
	emit_signal("drop_on_blacksmith", id)
