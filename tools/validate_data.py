...
def parse_int(value: str, file_name: str, line_number: int, column: str, report: Report, allow_empty: bool = False) -> int | None:
...
def validate_shop(file_name: str, rows: list[dict[str, str]], valid_item_ids: set[str], report: Report) -> None:
    require_columns(file_name, rows, {"id", "stock", "price_override"}, report)
    for line_number, row in enumerate(rows, start=2):
        item_id = row.get("id", "")
        if item_id not in valid_item_ids:
            report.error(f"data/{file_name}:{line_number} references unknown shop item '{item_id}'")
        stock = parse_int(row.get("stock", ""), file_name, line_number, "stock", report)
        if stock is not None and stock < -1:
            report.error(f"data/{file_name}:{line_number} stock must be -1 or greater")
        price_override = row.get("price_override", "")
        parse_int(price_override, file_name, line_number, "price_override", report, allow_empty=True)
...
def validate() -> int:
    ...
    shop_blacksmith_rows = read_csv("shop_blacksmith.csv", report)
    shop_healer_rows = read_csv("shop_healer.csv", report)
    shop_tavern_rows = read_csv("shop_tavern.csv", report)
    ...
    require_columns("items.csv", items_rows, {"id", "name", "kind", "subkind", "stackable", "max_stack", "price"}, report)
    ...
    all_catalog_item_ids = set(items) | set(weapons) | set(armors)
    loot_item_ids = all_catalog_item_ids | BUILTIN_LOOT_IDS
    ...
    validate_int_columns("items.csv", items_rows, {"heal_pct", "heal_hp", "max_stack", "price"}, report)
    ...
    for line_number, row in enumerate(loot_rows, start=2):
        ...
        if item_id not in loot_item_ids:
            report.error(f"data/loot_tables.csv:{line_number} references unknown item_id '{item_id}'")
        weight = parse_int(weight_raw, "loot_tables.csv", line_number, "weight", report)
    ...
    validate_shop("shop_blacksmith.csv", shop_blacksmith_rows, all_catalog_item_ids, report)
    validate_shop("shop_healer.csv", shop_healer_rows, all_catalog_item_ids, report)
    validate_shop("shop_tavern.csv", shop_tavern_rows, all_catalog_item_ids, report)
